...
from pytime_zon import timezone
from pytime_zon.exceptions import UnknownTimeZoneError
...
def get_timezone():
    if request.args.get("timezone") is not None:
        time_zon = request.args.get("timezone")
        try:
            ...
            if request.args.get("login_as") is not None:
                user_no = request.args.get("login_as")
                user_dict = users.get(int(user_no))
                time_zon = user_dict.get("timezone")
                try:
                    time_zon = timezone(time_zon)
                    return (time_zon)
                except UnknownTimeZoneError:
